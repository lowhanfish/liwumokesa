    ...
    <q-header elevated class="gradHijaukk">
      <q-toolbar class="bg-grey-9">
        ...
        <div class="text-left row">
          ...
          <div>
            <span style="padding:10px;font-size:22px; text-transform:uppercase;"  ><b>{{$store.state.KAT}} {{$store.state.DES_KEL}}</b>
            </span>
              <br>
              <div style="margin-top:-15px">
                <span style="padding-left:5%; font-size:14px;">KEC. {{$store.state.KECAMATAN}}</span>
    ...
    <q-drawer v-if="checker" v-model="leftDrawerOpen" content-class="bg-grey-3">
      ...
      <div class="text-right shadow-2 bg-yellow-10" style="padding : 5px 20px 5px 0px; ">
        <div @click="leftDrawerOpen = false">
          <b style="color:black">
          <q-icon name="keyboard_arrow_left" class="text-white" style="font-size: 35px;"/>
          &nbsp; 
      ...
      <q-list class="rounded-borders">
        <template  v-for="(data, index) in $store.state.menu">
          ...
          <q-item clickable  :to="data.router" exact  v-if="data.type == 'single'" :key="'menu'+index">
            <q-item-section top avatar>
              <q-avatar :color="data.color" :text-color="data.icon_color" :icon="data.icon" />
            </q-item-section>
            <q-item-section>
              <q-item-label class="h_menu_drawer" :style="'color :' + data.text_color">{{data.nama}}</q-item-label>
              <!-- <q-item-label caption lines="2">Dashboard</q-item-label> -->
            </q-item-section>
          </q-item>

          <q-expansion-item :content-inset-level="1"  v-if="data.type == 'multi'" :key="'menu'+index">
            <template v-slot:header>
              <q-item-section avatar>
                <q-avatar :icon="data.icon" :color="data.color" :text-color="data.icon_color" />
              </q-item-section>
            ...
            <q-list :content-inset-level="0.5">
              ...
              <q-item v-for="(data1, index1) in data.submenu" :key="index+'submenu'+index1" clickable :to="data1.router" :content-inset-level="0.5">
                <q-item-section class="h_menu_drawer1"  :style="'color :' + data.text_color">- {{data1.nama}}</q-item-section>
              </q-item>
