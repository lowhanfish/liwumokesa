                ...
                <q-markup-table separator="vertical" flat bordered style="width:100%">
                    ...
                    <tbody>
                        <tr v-for="(data, index) in list_data" :key="index"> 
                            ...
                            <td class="text-left"> {{data.masterAgama_uraian}} </td>
...
export default {
  ...
  methods: {
    getView : function(){
      ...
      fetch(this.$store.state.url.URL_CNT_DESA_DATA_AGAMA + "view", {
          method: "POST",
          headers: {
    ...
    data_Agama : async function(){
      var body = this.form
      var dataArray = await CHART.postData(this.$store.state.url.URL_CNT_DESA_DATA_AGAMA, 'char_data_agama', body);
    //   console.log(dataArray)
      CHART.BarChart('Potensi_Wilayah', 'Data Agama', 'Data Agama', dataArray.uraian, dataArray.jumlah, 'Orang') 
    },
