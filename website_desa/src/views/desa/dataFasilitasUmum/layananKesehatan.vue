                ...
                <q-markup-table separator="vertical" flat bordered style="width:100%">
                    ...
                    <tbody>
                        <tr v-for="(data, index) in list_data" :key="index" >
                            ...
                            <td class="text-left"> {{data.masterFasilitasKesehatan_uraian}} </td> 
...
export default {
  ...
  methods: {
    getView : function(){
      ...
      fetch(this.$store.state.url.URL_CNT_DESA_FASILITAS_KESEHATAN + "view", {
          method: "POST",
          headers: {
    ...
    chartnya : async function(){
      ...
      var dataArray = await CHART.postData(this.$store.state.url.URL_CNT_DESA_FASILITAS_KESEHATAN, 'char_fasilitas_kesehatan', body);
    //   console.log(dataArray)
      CHART.BarChart('barchartnya', 'Layanan Kesehatan', 'Data', dataArray.uraian, dataArray.jumlah, 'Unit') 
    },
