...
router.post('/', (req, res) => {
    ...
    let check = (
        ...
        filter datax.masterAgama == data.id && datax.des_kel == '`+req.body.des_kel+`' && datax.tahun == `+req.body.tahun+`
...
router.post('/view', (req, res) => {
    ...
    var query = `
        ...
        FOR jmlData in masterAgama
        FILTER jmlData.uraian LIKE '%`+cari+`%'
        COLLECT WITH COUNT INTO length

        let data = (
            FOR datax in masterAgama
...
router.post('/addData', (req,res)=>{
    ...
    var query = `
        insert `+reques+` into masterAgama
    `
    db.query(query)
    .then(cursor => cursor.all())
    .then((row, err)=>{
...
router.post('/editData',(req, res) => {
    ...
    // console.log(reques)
    var query = `
        FOR i in masterAgama
        FILTER i.id == '`+req.body.id+`'
        UPDATE i WITH `+reques+` IN masterAgama
    `
