...
router.post('/', (req, res) => {
    ...
    let check = (
        ...
        filter datax.masterJenisUsaha == data.id && datax.des_kel == '`+req.body.des_kel+`' && datax.tahun == `+req.body.tahun+`
...
router.post('/view', (req, res) => {
    ...
    var query = `
        ...
        FOR jmlData in masterJenisUsaha
        FILTER jmlData.uraian LIKE '%`+cari+`%'
        COLLECT WITH COUNT INTO length

        let data = (
            FOR datax in masterJenisUsaha
...
router.post('/addData', (req,res)=>{
    ...
    var query = `
        insert `+reques+` into masterJenisUsaha
    `
    db.query(query)
    .then(cursor => cursor.all())
    .then((row, err)=>{
...
router.post('/editData',(req, res) => {
    ...
    // console.log(reques)
    var query = `
        FOR i in masterJenisUsaha
        FILTER i.id == '`+req.body.id+`'
        UPDATE i WITH `+reques+` IN masterJenisUsaha
    `
...
router.post('/removeData', (req, res)=> {
    ...
    var query = `
        ...
        REMOVE data IN masterJenisUsaha
    `
