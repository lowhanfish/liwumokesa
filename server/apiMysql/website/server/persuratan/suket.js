...
router.post('/addData', upload.single("file"), (req, res) => {
    ...
    var query = `
        insert `+ reques + ` into suketAhliWaris
    `
    db.query(query)
        .then(cursor => cursor.all())
        .then((row, err) => {
...
router.post('/editData', upload.single("file"), (req, res) => {
    ...
    var query = `
        FOR i in suketAhliWaris
        FILTER i.id == '`+ form.id + `'
        UPDATE i WITH `+ reques + ` IN suketAhliWaris
    `
    // console.log(query)
...
function generateCard (req, res, idz) {
        ...
        var externalURL = urlServer+`cnt_suketAhliWaris?id=`+IDX
