...
router.post('/view', (req, res) => {
    ...
    var query = `
        ...
        FOR jmlData in desa_dok_lainnya
...
router.post('/addData', upload.single("file"), (req, res) => {
    ...
    var query = `
        insert `+ reques + ` into desa_dok_lainnya
    `
    db.query(query)
        .then(cursor => cursor.all())
        .then((row, err) => {
...
router.post('/editData', upload.single("file"), (req, res) => {
    ...
    var query = `
        FOR i in desa_dok_lainnya
        FILTER i.id == '`+ req.body.id + `'
        UPDATE i WITH `+ reques + ` IN desa_dok_lainnya
    `
    // console.log(query)
...
router.post('/removeData', (req, res) => {
    ...
    console.log(req.body)
    var query = `
        FOR data IN desa_dok_lainnya
        filter data.id == '`+ req.body.id + `'
        REMOVE data IN desa_dok_lainnya
    `
