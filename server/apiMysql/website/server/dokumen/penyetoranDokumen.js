...
router.post('/view', (req, res) => {
    ...
    if (req.body.kecamatan == undefined || req.body.kecamatan == null || req.body.kecamatan == false || req.body.kecamatan== '') {
        ...
        if (req.body.des_kel == undefined || req.body.des_kel == null || req.body.des_kel == ""){
            ...
            cari_des_kel = `FILTER datax.uniqid == '`+req.body.des_kel+`'`
            jmlData_des_kel = `FILTER jmlData.uniqid == '`+req.body.des_kel+`'`
        }
    ...
    var query = `
        ...
        FOR jmlData IN desa_dok_permintaanDokumen_desa
        `+jmlData_des_kel+`

        for permintaan in desa_dok_permintaanDokumen
        filter jmlData.desa_dok_permintaanDokumen_id == permintaan.id


        for jmlDes_kel in des_kel
        filter jmlDes_kel.uniqid == jmlData.uniqid

        for jmlKec in kecamatan
        filter jmlKec.id == jmlDes_kel.kecamatan

        for user in users
        filter user.id == permintaan.createdBy
        ...
        let data = (
            ...
            FOR datax IN desa_dok_permintaanDokumen_desa
            `+cari_des_kel+`

            for permintaan in desa_dok_permintaanDokumen
            filter datax.desa_dok_permintaanDokumen_id == permintaan.id


            for des_kel in des_kel
            filter des_kel.uniqid == datax.uniqid
            ...
            RETURN {
                ...
                uraian : permintaan.uraian,
                tahun : permintaan.tahun,
                tgl_dari : permintaan.tgl_dari,
                tgl_sampai : permintaan.tgl_sampai,
                des_kel : permintaan.des_kel,
                des_kel_uraian : des_kel.uraian,
                kecamatan : permintaan.kecamatan,
                kecamatan_uraian : kec.uraian,
                kelasFile : permintaan.kelasFile,
                createdAt : permintaan.createdAt,
                createdBy : permintaan.createdBy,
...
router.post('/view_file', (req, res) => {
    ...
    var query = `
        ...
        FILTER 
        datax.desa_dok_permintaanDokumen_id == '`+req.body.desa_dok_permintaanDokumen_id+`' 
        && datax.des_kel == '`+req.body.des_kel+`'
