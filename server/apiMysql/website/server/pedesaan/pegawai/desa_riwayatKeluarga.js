...
router.get('/', (req, res) => {
    let view = `
        SELECT * FROM pamong_riwayat_riwayatKeluarga;
    `;
...
router.post('/view', (req, res) => {
    ...
    var query = `
        ...
        FOR jmlData in pamong_riwayat_riwayatKeluarga
        ...
        let data = (
            FOR datax in pamong_riwayat_riwayatKeluarga
...
router.post('/addData', upload.single("file"), (req, res) => {
    ...
    var query = `
        insert `+ reques + ` into pamong_riwayat_riwayatKeluarga
    `
    db.query(query)
        .then(cursor => cursor.all())
        .then((row, err) => {
...
router.post('/editData', upload.single("file"), (req, res) => {
    ...
    var query = `
        FOR i in pamong_riwayat_riwayatKeluarga
        FILTER i.id == '`+ req.body.id + `'
        UPDATE i WITH `+ reques + ` IN pamong_riwayat_riwayatKeluarga
    `
    // console.log(query)
...
router.post('/removeData', (req, res) => {
    ...
    // console.log(req.body)
    var query = `
        FOR data IN pamong_riwayat_riwayatKeluarga
        filter data.id == '`+ req.body.id + `'
        REMOVE data IN pamong_riwayat_riwayatKeluarga
    `
