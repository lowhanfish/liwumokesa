...
router.get('/', (req, res) => {
    let view = `
        SELECT * FROM pamong_riwayat_riwayatPrestasi;
    `;
...
router.post('/view', (req, res) => {
    ...
    var query = `
        ...
        FOR jmlData in pamong_riwayat_riwayatPrestasi
        ...
        let data = (
            FOR datax in pamong_riwayat_riwayatPrestasi
...
router.post('/addData', upload.single("file"), (req, res) => {
    ...
    var query = `
        insert `+ reques + ` into pamong_riwayat_riwayatPrestasi
    `
    db.query(query)
        .then(cursor => cursor.all())
        .then((row, err) => {
...
router.post('/editData', upload.single("file"), (req, res) => {
    ...
    var query = `
        FOR i in pamong_riwayat_riwayatPrestasi
        FILTER i.id == '`+ req.body.id + `'
        UPDATE i WITH `+ reques + ` IN pamong_riwayat_riwayatPrestasi
    `
    // console.log(query)
...
router.post('/removeData', (req, res) => {
    ...
    // console.log(req.body)
    var query = `
        FOR data IN pamong_riwayat_riwayatPrestasi
        filter data.id == '`+ req.body.id + `'
        REMOVE data IN pamong_riwayat_riwayatPrestasi
    `
