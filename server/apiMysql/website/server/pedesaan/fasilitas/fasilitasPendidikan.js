...
router.get('/', (req, res) => {
    let view = `
        SELECT * FROM desa_fasilitasPendidikan;
    `;
...
router.post('/view', (req, res) => {
    ...
    var query = `
        ...
        FOR jmlData in desa_fasilitasPendidikan
        FILTER jmlData.tahun == `+req.body.tahun+` `+cari_des_kel_jml+` 
        COLLECT WITH COUNT INTO length

        let data = (
            FOR datax in desa_fasilitasPendidikan
            for dataxx in masterFasilitasPendidikan
            FILTER datax.masterFasilitasPendidikan ==  dataxx.id && datax.tahun == `+req.body.tahun+` `+cari_des_kel+` 
            ...
            RETURN {
                ...
                tahun : datax.tahun,
                masterFasilitasPendidikan : datax.masterFasilitasPendidikan,
                jumlah :datax.jumlah,
                masterFasilitasPendidikan_uraian : dataxx.uraian,
...
router.post('/addData', (req,res)=>{
    ...
    datax.forEach(element => {
        var insert = {
            ...
            tahun : form.tahun,
            masterFasilitasPendidikan : element.id,
            jumlah : element.jumlah,
        }
        var reques = JSON.stringify(insert)
        var query = `
            insert `+reques+` into desa_fasilitasPendidikan
        `
        db.query(query)
        .then(cursor => cursor.all())
        .then((row, err)=>{
...
router.post('/editData',(req, res) => {
    ...
    // console.log(reques)
    var query = `
        FOR i in desa_fasilitasPendidikan
        FILTER i.id == '`+req.body.id+`'
        UPDATE i WITH `+reques+` IN desa_fasilitasPendidikan
    `
...
router.post('/removeData', (req, res)=> {
    var reques = JSON.stringify(req.body)
    var query = `
        FOR data IN desa_fasilitasPendidikan
        filter data.id == '`+req.body.id+`'
        REMOVE data IN desa_fasilitasPendidikan
    `
