...
router.get('/', (req, res) => {
    let view = `
        SELECT * FROM desa_fasilitasUmum;
    `;
...
router.post('/view', (req, res) => {
    ...
    var query = `
        ...
        FOR jmlData in desa_fasilitasUmum
        FILTER jmlData.tahun == `+req.body.tahun+` `+cari_des_kel_jml+` 
        COLLECT WITH COUNT INTO length

        let data = (
            FOR datax in desa_fasilitasUmum
            for dataxx in masterFasilitasUmum
            FILTER datax.masterFasilitasUmum ==  dataxx.id && datax.tahun == `+req.body.tahun+` `+cari_des_kel+` 
            ...
            RETURN {
                ...
                tahun : datax.tahun,
                masterFasilitasUmum : datax.masterFasilitasUmum,
                jumlah :datax.jumlah,
                masterFasilitasUmum_uraian : dataxx.uraian,
...
router.post('/addData', (req,res)=>{
    ...
    datax.forEach(element => {
        var insert = {
            ...
            tahun : form.tahun,
            masterFasilitasUmum : element.id,
            jumlah : element.jumlah,
        }
        var reques = JSON.stringify(insert)
        var query = `
            insert `+reques+` into desa_fasilitasUmum
        `
        db.query(query)
        .then(cursor => cursor.all())
        .then((row, err)=>{
...
router.post('/editData',(req, res) => {
    ...
    // console.log(reques)
    var query = `
        FOR i in desa_fasilitasUmum
        FILTER i.id == '`+req.body.id+`'
        UPDATE i WITH `+reques+` IN desa_fasilitasUmum
    `
...
router.post('/removeData', (req, res)=> {
    var reques = JSON.stringify(req.body)
    var query = `
        FOR data IN desa_fasilitasUmum
        filter data.id == '`+req.body.id+`'
        REMOVE data IN desa_fasilitasUmum
    `
