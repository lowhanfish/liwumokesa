...
router.get('/', (req, res) => {
    let view = `
        SELECT * FROM desa_fasilitasPemerintahan;
    `;
...
router.post('/view', (req, res) => {
    ...
    var query = `
        ...
        FOR jmlData in desa_fasilitasPemerintahan
        FILTER jmlData.tahun == `+req.body.tahun+` `+cari_des_kel_jml+` 
        COLLECT WITH COUNT INTO length

        let data = (
            FOR datax in desa_fasilitasPemerintahan
            for dataxx in masterFasilitasPemerintahan
            FILTER datax.masterFasilitasPemerintahan ==  dataxx.id && datax.tahun == `+req.body.tahun+` `+cari_des_kel+` 
            ...
            RETURN {
                ...
                tahun : datax.tahun,
                masterFasilitasPemerintahan : datax.masterFasilitasPemerintahan,
                jumlah :datax.jumlah,
                masterFasilitasPemerintahan_uraian : dataxx.uraian,
...
router.post('/addData', (req,res)=>{
    ...
    datax.forEach(element => {
        var insert = {
            ...
            tahun : form.tahun,
            masterFasilitasPemerintahan : element.id,
            jumlah : element.jumlah,
        }
        var reques = JSON.stringify(insert)
        var query = `
            insert `+reques+` into desa_fasilitasPemerintahan
        `
        db.query(query)
        .then(cursor => cursor.all())
        .then((row, err)=>{
...
router.post('/editData',(req, res) => {
    ...
    // console.log(reques)
    var query = `
        FOR i in desa_fasilitasPemerintahan
        FILTER i.id == '`+req.body.id+`'
        UPDATE i WITH `+reques+` IN desa_fasilitasPemerintahan
    `
...
router.post('/removeData', (req, res)=> {
    var reques = JSON.stringify(req.body)
    var query = `
        FOR data IN desa_fasilitasPemerintahan
        filter data.id == '`+req.body.id+`'
        REMOVE data IN desa_fasilitasPemerintahan
    `
