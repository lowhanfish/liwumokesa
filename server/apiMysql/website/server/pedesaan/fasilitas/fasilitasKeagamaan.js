...
router.post('/addData', (req,res)=>{
    ...
    datax.forEach(element => {
        var insert = {
            ...
            tahun : form.tahun,
            masterFasilitasKeagamaan : element.id,
            jumlah : element.jumlah,
        }
        var reques = JSON.stringify(insert)
        var query = `
            insert `+reques+` into desa_fasilitasAgama
        `
        db.query(query)
        .then(cursor => cursor.all())
        .then((row, err)=>{
...
router.post('/editData',(req, res) => {
    ...
    // console.log(reques)
    var query = `
        FOR i in desa_fasilitasAgama
        FILTER i.id == '`+req.body.id+`'
        UPDATE i WITH `+reques+` IN desa_fasilitasAgama
    `
...
router.post('/removeData', (req, res)=> {
    ...
    var query = `
        ...
        REMOVE data IN desa_fasilitasAgama
    `
