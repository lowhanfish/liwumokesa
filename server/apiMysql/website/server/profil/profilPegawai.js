...
router.post('/view', (req, res) => {
    ...
    var query = `
        ...
        FOR jmlData in profilPegawai
        FILTER jmlData.uraian LIKE '%`+cari+`%'
        COLLECT WITH COUNT INTO length

        let data = (
            FOR datax in profilPegawai
...
router.post('/addData', (req,res)=>{
    ...
    var query = `
        insert `+reques+` into profilPegawai
    `
    db.query(query)
    .then(cursor => cursor.all())
    .then((row, err)=>{
...
router.post('/editData',(req, res) => {
    ...
    // console.log(reques)
    var query = `
        FOR i in profilPegawai
        FILTER i.id == '`+req.body.id+`'
        UPDATE i WITH `+reques+` IN profilPegawai
    `
...
router.post('/removeData', (req, res)=> {
    var reques = JSON.stringify(req.body)
    var query = `
        FOR data IN profilPegawai
        filter data.id == '`+req.body.id+`'
        REMOVE data IN profilPegawai
    `
