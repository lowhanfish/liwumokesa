...
router.post('/addData', upload.single("file"), (req, res) => {
    ...
    var query = `
        insert `+ reques + ` into galleryFoto
    `
    db.query(query)
        .then(cursor => cursor.all())
        .then((row, err) => {
...
router.post('/editData', upload.single("file"), (req, res) => {
    ...
    var query = `
        FOR i in galleryFoto
        FILTER i.id == '`+ req.body.id + `'
        UPDATE i WITH `+ reques + ` IN galleryFoto
    `
    // console.log(query)
...
router.post('/removeData', (req, res) => {
    ...
    console.log(req.body)
    var query = `
        FOR data IN galleryFoto
        filter data.id == '`+ req.body.id + `'
        REMOVE data IN galleryFoto
    `
