...
router.post('/editData', upload.single("file"), (req, res) => {
    ...
    var query = `
        FOR i in suketUsaha
        FILTER i.id == '`+ form.id + `'
        UPDATE i WITH `+ reques + ` IN suketUsaha
    `
    // console.log(query)
...
function generateCard (req, res, idz) {
        ...
        var externalURL = urlServer+`cnt_suketUsaha?id=`+IDX
