...
router.post('/view', (req, res) => {
    ...
    var query = `
        ...
        FOR jmlData in desa_jenisUsaha
        FILTER jmlData.tahun == `+req.body.tahun+` `+cari_des_kel_jml+` 
        COLLECT WITH COUNT INTO length

        let data = (
            FOR datax in desa_jenisUsaha
            for dataxx in masterJenisUsaha
            FILTER datax.masterJenisUsaha ==  dataxx.id && datax.tahun == `+req.body.tahun+` `+cari_des_kel+` 
            ...
            RETURN {
                ...
                tahun : datax.tahun,
                masterJenisUsaha : datax.masterJenisUsaha,
                jumlah :datax.jumlah,
                masterJenisUsaha_uraian : dataxx.uraian,
...
router.post('/char_jenis_usaha', (req, res) => {

    var cari_des_kel = ''
    if (req.body.des_kel == undefined || req.body.des_kel == null || req.body.des_kel == ""){
        cari_des_kel = ''
    }else{
        cari_des_kel = `&& jen_usaha.des_kel == '`+req.body.des_kel+`'`
    }
    
    var query = `
    for data in masterJenisUsaha
    let jusaha = (
        for jen_usaha in desa_jenisUsaha
        filter data.id == jen_usaha.masterJenisUsaha
        filter jen_usaha.tahun == `+req.body.tahun+` `+cari_des_kel+`  
        return TO_NUMBER(jen_usaha.jumlah)
