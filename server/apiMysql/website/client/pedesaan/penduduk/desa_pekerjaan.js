...
router.post('/view', (req, res) => {
    ...
    var query = `
        ...
        FOR jmlData in desa_pekerjaan
        FILTER jmlData.tahun == `+req.body.tahun+` `+cari_des_kel_jml+` 
        COLLECT WITH COUNT INTO length

        let data = (
            FOR datax in desa_pekerjaan
            for dataxx in masterPekerjaan
            FILTER datax.masterPekerjaan ==  dataxx.id && datax.tahun == `+req.body.tahun+` `+cari_des_kel+` 
            ...
            RETURN {
                ...
                tahun : datax.tahun,
                masterPekerjaan : datax.masterPekerjaan,
                jumlah :datax.jumlah,
                masterPekerjaan_uraian : dataxx.uraian,
...
// char_data_pekerjaan
router.post('/char_data_pekerjaan', (req, res) => {

    var cari_des_kel = ''
    if (req.body.des_kel == undefined || req.body.des_kel == null || req.body.des_kel == ""){
        cari_des_kel = ''
    }else{
        cari_des_kel = `&& jen_pekerjaan.des_kel == '`+req.body.des_kel+`'`
    }
    
    var query = `
    for data in masterPekerjaan
    let pekerjaan = (
        for jen_pekerjaan in desa_pekerjaan
        filter data.id == jen_pekerjaan.masterPekerjaan
        filter jen_pekerjaan.tahun == `+req.body.tahun+` `+cari_des_kel+`  
        return TO_NUMBER(jen_pekerjaan.jumlah)
    
    )
    return {
        uraian : data.uraian,
        jumlah : SUM(pekerjaan)
    } 
