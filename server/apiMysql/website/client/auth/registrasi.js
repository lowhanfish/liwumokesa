...
router.post('/signup', (req, res, next) => {
    ...
    if (result.error === null) {
        ...
        let view = `
            for data in users
            filter data.username == '`+req.body.username+`' || data.email == '`+req.body.email+`' || data.hp == '`+req.body.hp+`'
            return data
        `
        ...
        .then((row, err) => {
            if(row.length <= 0) {
                ...
                const error = new Error('Username, hp atau email sudah digunakan sebelumnya');
                res.status(409);
                next(error);
            }
        });
