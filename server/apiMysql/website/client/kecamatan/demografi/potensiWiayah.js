...
router.get('/', (req, res) => {
    let view = `
        SELECT * FROM desa_potensiWiayah;
    `;
...
router.post('/view', (req, res) => {
    ...
    var query = `
        ...
        FOR jmlData in desa_potensiWiayah
        FILTER jmlData.tahun == `+req.body.tahun+` `+cari_des_kel_jml+` 
        COLLECT WITH COUNT INTO length

        let data = (
            FOR datax in desa_potensiWiayah
            for dataxx in masterPotensiWilayah
            FILTER datax.masterPotensiWilayah ==  dataxx.id && datax.tahun == `+req.body.tahun+` `+cari_des_kel+` 
            ...
            RETURN {
                ...
                tahun : datax.tahun,
                masterPotensiWilayah : datax.masterPotensiWilayah,
                jumlah :datax.jumlah,
                masterPotensiWilayah_uraian : dataxx.uraian,
...
router.post('/addData', (req,res)=>{
    ...
    datax.forEach(element => {
        var insert = {
            ...
            tahun : form.tahun,
            masterPotensiWilayah : element.id,
            jumlah : element.jumlah,
        }
        var reques = JSON.stringify(insert)
        var query = `
            insert `+reques+` into desa_potensiWiayah
        `
        db.query(query)
        .then(cursor => cursor.all())
        .then((row, err)=>{
...
router.post('/editData',(req, res) => {
    ...
    // console.log(reques)
    var query = `
        FOR i in desa_potensiWiayah
        FILTER i.id == '`+req.body.id+`'
        UPDATE i WITH `+reques+` IN desa_potensiWiayah
    `
...
router.post('/removeData', (req, res)=> {
    var reques = JSON.stringify(req.body)
    var query = `
        FOR data IN desa_potensiWiayah
        filter data.id == '`+req.body.id+`'
        REMOVE data IN desa_potensiWiayah
    `
