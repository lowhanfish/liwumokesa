...
router.post('/addData', (req,res)=>{
    ...
    datax.forEach(element => {
        var insert = {
            ...
            tahun : form.tahun,
            masterPekerjaan : element.id,
            jumlah : element.jumlah,
        }
        var reques = JSON.stringify(insert)
        var query = `
            insert `+reques+` into desa_pekerjaan
        `
        db.query(query)
        .then(cursor => cursor.all())
        .then((row, err)=>{
...
router.post('/editData',(req, res) => {
    ...
    // console.log(reques)
    var query = `
        FOR i in desa_pekerjaan
        FILTER i.id == '`+req.body.id+`'
        UPDATE i WITH `+reques+` IN desa_pekerjaan
    `
...
router.post('/char_fasilitas_pendidikan', (req, res) => {
    ...
    let datax = (
        for dataz in desa_fasilitasPendidikan
        filter data.id == dataz.masterFasilitasPendidikan
