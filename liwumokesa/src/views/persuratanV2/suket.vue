      ...
      <q-card-section class="bg-grey-7 text-white">
        <div class="row">
          <div class="col-12 col-md-6">
            <div class="text-h6 h_titleHead">Suket Lainnya</div>
          ...
          <!-- =================================================== KONTENT =========================================================== -->
            <table width="100%">
              ...
              <tr  :class="'h_table_body '+UMUM.StatusIndikator(data.data.status)" v-for="(data, index) in list_data" :key="data._key">
                ...
                <td>{{data.masterjenissuket.jenis}}</td>
                <td class="text-center">{{UMUM.tglConvertXY(data.data.createdAt)}}</td>
                              ...
                              <q-card-section class="q-pt-none">
                                ...
                                <div class="row">
                                  <div class="col-12" v-if="form.status == 0 || this.form.status == 2">
                                      ...
                                      <iframe :src=" 'https://view.officeapps.live.com/op/embed.aspx?src=' +$store.state.url.URL_APP +'uploads/' +fileOld" width="100%" height="565px" frameborder="0">
                                      </iframe>
...
export default {
  ...
  methods: {
    ...
    getView : function(){
      ...
      fetch(this.$store.state.url.URL_srv_suketLainnya + "viewMaster", {
          method: "POST",
          headers: {
    ...
    approveData : function(number) {
      ...
      if (this.form.no == '' || this.form.no == ' '|| this.form.no == null || this.form.no == undefined) {
        ...
        this.form.KODE_NOMOR  = this.form.no

        this.form.editedAt = UMUM.NOW()
        fetch(this.$store.state.url.URL_srv_suketLainnya + "verifikasi", {
            method: "POST",
            headers: {
              "content-type": "application/json",
              authorization: "kikensbatara " + localStorage.token
    ...
    selectData : function(data){
        ...
        this.form.fileOld = data.data.file;
        ...
        this.form.KODE_KECAMATAN = data.kecamatan.uraian
        this.form.KODE_DESA  = data.desa.uraian
        this.form.KODE_ALAMAT  = data.desa.alamat
        this.form.KODE_NTTD  = data.pegawai.nama
        this.form.KODE_TTD  = this.$store.state.url.URL_APP+'uploads/'+this.fileOld

        console.log(this.form.KODE_TTD);
        console.log(data)
    },
  ...
  mounted () {
    ...
    this.form.KODE_TANGGAL = UMUM.tglConvertX(tgl)

    // console.log(this.form.KODE_TANGGAL);
