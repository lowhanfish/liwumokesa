                ...
                <q-markup-table separator="vertical" flat bordered style="width:100%">
                    ...
                    <tbody>
                        <tr v-for="(data, index) in list_data" :key="index">
                            ...
                            <td class="text-left">Usia {{data.uraian}} </td>
...
export default {
  ...
  methods: {
    getView : async function(){
      var body = this.form
      this.list_data = await FETCHING.postDataPublish(this.$store.state.url.URL_CNT_KECAMATAN_KECAMATAN, 'PiramidaPenduduk', body);
      console.log(this.list_data)
    },
    chartnya : async function(){
      var body = this.form
      var dataArray = await CHART.postData(this.$store.state.url.URL_CNT_KECAMATAN_DASHBOARD, 'PiramidaPenduduk', body);
      console.log(dataArray)
      CHART.BarChart('barchartnya', 'Data Piramida Penduduk', '', dataArray.uraian , dataArray.jumlah, 'Orang')
    },
    // ====================================== PAGINATE ====================================
