      ...
      <q-card-section class="bg-grey-7 text-white">
        <div class="row">
          <div class="col-12 col-md-6">
            <div class="text-h6 h_titleHead">Master Jenis Surat</div>
            <div class="text-subtitle2">Persuratan</div>
          </div>
        ...
        <div class="tbl_responsive">
          ...
          <table width="100%">
            <tr class="h_table_head bg-grey-5">
              ...
              <th width="35%" class="">Jenis Surat</th>
              <th width="50%" class="">Keterangan</th>
              <th width="10%"></th>
            </tr>
            <tr
              class="h_table_body"
              v-for="(data, index) in list_data"
              :key="data.id"
            >
              ...
              <td class="h_top">{{ data.keterangan }}</td>
                ...
                <q-btn-group>
                  ...
                  >
                    <q-tooltip
                      ...
                      content-style="font-size: 13px"
                    >
                      Click untuk melihat template surat
                    </q-tooltip>
                  </q-btn>
                  <q-btn
                    @click="
    ...
    <q-dialog v-model="mdl_add" persistent>
      <q-card class="mdl-md">
        <q-card-section class="bg-grey-7">
          <div class="text-h6 h_modalhead">Tambah Produk HUkum</div>
        </q-card-section>

        <form @submit.prevent="addData()">
          <q-card-section class="q-pt-none">
            <br />
            <span class="h_lable ">Jenis Surat</span>
            <q-input
              v-model="form.jenis"
              outlined
              square
    ...
    <q-dialog v-model="mdl_edit" persistent>
      <q-card class="mdl-md">
        <q-card-section class="bg-orange">
          <div class="text-h6 h_modalhead">Edit Produk HUkum</div>
        </q-card-section>
      ...
      <q-card class="mdl-lg">
        ...
        <form @submit.prevent="addData()">
          <q-card-section class="q-pt-none">
            ...
            &nbsp;
            <span class="h_panation"
              >({{ UMUM.tglConvert(form.createdAt) }})</span
            >
            ...
            <!-- <iframe  :src="'http://docs.google.com/gview?url='+fileWord+'&embedded=true'" height="240" width="320" frameborder="0"></iframe> -->
            <iframe
              :src="
                'https://view.officeapps.live.com/op/embed.aspx?src=' +
                  $store.state.url.URL_APP +
                  'uploads/' +
                  fileOld
              "
              width="100%"
...
export default {
  data() {
    return {
      ...
      fileOld: "",
      fileWord:
        "http://www.learningaboutelectronics.com/Articles/NP-modernization-act-new-york-state.doc",
  ...
  methods: {
    getView: function() {
      ...
      fetch(this.$store.state.url.URL_srv_masterJenisSuket + "view", {
        method: "POST",
        headers: {
    ...
    addData: function(number) {
      ...
      fetch(this.$store.state.url.URL_srv_masterJenisSuket + "addData", {
        method: "POST",
        headers: {
          // "content-type": "application/json",
          authorization: "kikensbatara " + localStorage.token,
    ...
    editData: function() {
      ...
      fetch(this.$store.state.url.URL_srv_masterJenisSuket + "editData", {
        method: "POST",
        headers: {
          // "content-type": "application/json",
          authorization: "kikensbatara " + localStorage.token,
    ...
    removeData: function(E) {
      fetch(this.$store.state.url.URL_srv_masterJenisSuket + "removeData", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          authorization: "kikensbatara " + localStorage.token,
