                ...
                <q-markup-table separator="vertical" flat bordered style="width:100%">
                    ...
                    <tbody>
                        <tr v-for="(data, index) in list_data" :key="index" >
                            ...
                            <td class="text-left"> {{data.masterFasilitasPemerintahan_uraian}} </td> 
...
// char_fasilitas_pendidikan
    getView : function(){
      ...
      fetch(this.$store.state.url.URL_CNT_DESA_FASILITAS_PEMERINTAHAN + "view", {
          method: "POST",
          headers: {
    ...
    chartnya : async function(){
      ...
      var dataArray = await CHART.postData(this.$store.state.url.URL_CNT_DESA_FASILITAS_PEMERINTAHAN, 'chart', body);
    //   console.log(dataArray)
      CHART.BarChart('barchartnya', 'Layanan Pemerintahan', 'Data', dataArray.uraian, dataArray.jumlah, 'Unit') 
    },
