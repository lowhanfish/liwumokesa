                ...
                <q-markup-table separator="vertical" flat bordered style="width:100%">
                    ...
                    <tbody>
                        <tr v-for="(data, index) in list_data" :key="index" >
                            ...
                            <td class="text-left"> {{data.masterFasilitasPendidikan_uraian}} </td> 
...
export default {
  ...
  methods: {
// URL_CNT_DESA_FASILITAS_PENDIDIKAN
// char_fasilitas_pendidikan
    getView : function(){
      this.$store.commit("shoWLoading");
      fetch(this.$store.state.url.URL_CNT_DESA_FASILITAS_PENDIDIKAN + "view", {
          method: "POST",
          headers: {
    ...
    chartnya : async function(){
      var body = this.form
      var tahun = this.form.tahun
      var dataArray = await CHART.postData(this.$store.state.url.URL_CNT_DESA_FASILITAS_PENDIDIKAN, 'char_fasilitas_pendidikan', body);
    //   console.log(dataArray)
      CHART.BarChart('barchartnya', 'Layanan Pendidikan', 'Data', dataArray.uraian, dataArray.jumlah, 'Unit') 
    },
