                ...
                <q-markup-table separator="vertical" flat bordered style="width:100%">
                    ...
                    <tbody>
                        <tr v-for="(data, index) in list_data" :key="index" >
                            ...
                            <td class="text-center"> {{data.masterFasilitasKeagamaan_uraian}} </td> 
...
export default {
  ...
  methods: {
    getView : function(){
      ...
      fetch(this.$store.state.url.URL_CNT_DESA_FASILITAS_KEAGAMAAN + "view", {
          method: "POST",
          headers: {
    ...
    Layanan_keagamaan : async function(){
      var body = this.form
      var dataArray = await CHART.postData(this.$store.state.url.URL_CNT_DESA_FASILITAS_KEAGAMAAN, 'char_fasilitas_agama', body);
      console.log(dataArray)
      CHART.BarChart('barchartnya', 'Layanan Keagamaan', 'Data', dataArray.uraian, dataArray.jumlah, 'Unit') 
    },
