                    ...
                    <q-markup-table separator="vertical" flat bordered style="width:100%">
                        ...
                        <tbody>
                            <tr v-for="(data, index) in list_data" :key="index"> 
                                ...
                                <td class="text-center"> {{data.masterJenisUsaha_uraian}} </td> 
...
export default {
  ...
  methods: {
    ...
    //   this.$store.commit("shoWLoading");
      fetch(this.$store.state.url.URL_CNT_DESA_JENIS_USAHA + "view", {
          method: "POST",
          headers: {
    ...
    // char_jenis_usaha
    // ====================================== CHART
    Jenis_Usaha : async function(){
      var body = this.form
      var dataArray = await CHART.postData(this.$store.state.url.URL_CNT_DESA_JENIS_USAHA, 'char_jenis_usaha', body);
      console.log(dataArray)
      CHART.BarChart('Potensi_Wilayah', 'Jenis Usaha', 'Data Jenis Usaha', dataArray.uraian, dataArray.jumlah, 'Orang') 
    },
    // ====================================== CHART
  },
  mounted() {
