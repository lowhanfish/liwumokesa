                ...
                <div class="row">
                    ...
                    <div class="col-12 col-md-6" style="padding : 0% 1% 2% 1%">
                        <select v-model="form.des_kel" @change="cari_data()" class="bg-white"  outlined square :dense="true" style="width:90%" disabled>
                            ...
                            <option v-for="data in $store.state.listDesKel" :key="data.uniqid" :value="data.uniqid" @click="selectData(data)">{{data.uraian}}</option>
                        </select>
                    </div>
                </div>

                <hr class="hrdetilberita" /> 
               <div id="Data_Pendidikan" style="width:100%; height:400px;">
                ...
                <q-markup-table separator="vertical" flat bordered style="width:100%">
                    ...
                    <tbody>
                        <tr v-for="(data, index) in list_data" :key="index"> 
                            ...
                            <td class="text-center"> {{data.masterPendidikan_uraian}} </td>
...
export default {
  ...
  methods: {
    getView : function(){
      ...
      fetch(this.$store.state.url.URL_CNT_DESA_DATA_PENDIDIKAN + "view", {
          method: "POST",
          headers: {
    ...
    // Data_Pendidikan
    chartDataPendidikan : async function(){
      var body = this.form
      var dataArray = await CHART.postData(this.$store.state.url.URL_CNT_DESA_DATA_PENDIDIKAN, 'chart_data_pendidikan', body);
      console.log(dataArray)
      CHART.BarChart('Data_Pendidikan', 'Data Pendidikan', 'Data Pendidikan', dataArray.uraian, dataArray.jumlah, 'Orang')
                                    // id,             title,               subtitle,       categories,    data
    },
