                ...
                <q-markup-table separator="vertical" flat bordered style="width:100%">
                    ...
                    <tbody>
                        <tr v-for="(data, index) in list_data" :key="index"> 
                            ...
                            <td class="text-center"> {{data.masterPekerjaan_uraian}} </td> 
...
export default {
  ...
  methods: {
    getView : function(){
      ...
      fetch(this.$store.state.url.URL_CNT_DESA_DATA_PEKERJAAN + "view", {
          method: "POST",
          headers: {
    ...
    data_Pekerjaan : async function(){
      var body = this.form
      var dataArray = await CHART.postData(this.$store.state.url.URL_CNT_DESA_DATA_PEKERJAAN, 'char_data_pekerjaan', body);
    //   console.log(dataArray)
      CHART.BarChart('Potensi_Wilayah', 'Jenis Pekerjaan', 'Data Jenis Pekerjaan', dataArray.uraian, dataArray.jumlah, 'Orang') 
    },
