      ...
      <q-card-section class="bg-grey-7 text-white">
        <div class="row">
          <div class="col-12 col-md-6">
            <div class="text-h6 h_titleHead">Suket Belum Memiliki Rumah</div>
...
export default {
  ...
  methods: {
    ...
    getView : function(){
      ...
      fetch(this.$store.state.url.URL_srv_suketTidakMemilikiRumah + "viewMaster", {
          method: "POST",
          headers: {
    ...
    approveData : function(number) {
      ...
      if (this.form.no == '' || this.form.no == ' '|| this.form.no == null || this.form.no == undefined) {
        ...
        fetch(this.$store.state.url.URL_srv_suketTidakMemilikiRumah + "verifikasi", {
            method: "POST",
            headers: {
              "content-type": "application/json",
              authorization: "kikensbatara " + localStorage.token
