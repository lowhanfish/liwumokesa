      ...
      <q-card-section class="bg-light-green-7 text-white">
        <div class="row">
          <div class="col-12 col-md-6">
            <div class="text-h6 h_titleHead">Batas dan Luas Wilayah</div>
            <div class="text-subtitle2">Data Kecamatan</div>
          </div>
          <div class="col-12 col-md-2"></div>
          <div class="col-12 col-md-4">
            <div class="row">
              <q-input v-model="cari_value" @keyup="cari_data()" outlined square :dense="true" class="bg-white" style="width:90%" disabled/>
          ...
          <!-- =================================================== KONTENT =========================================================== -->
            <table width="100%">
              <tr class="h_table_head bg-grey-5">
                ...
                <th width="11%" class="text-center">Luas</th>
                <th width="18%">Batas Utara</th>
                <th width="18%">Batas Selatan</th>
                <th width="18%">Batas Barat</th>
                <th width="18%">Batas Timur</th>
                <th width="12%"></th>
              </tr>
              <tr class="h_table_body" v-for="(data, index) in list_data" :key="data._key">
                <td class="text-center">{{indexing(index+1)}}.</td>
                <td class="text-center">{{data.luas}} Ha</td>
                <td>{{data.utara}}</td>
                <td>{{data.selatan}}</td>
                <td>{{data.barat}}</td>
                <td>{{data.timur}}</td>
                ...
                <q-card class="mdl-md">
                  ...
                  <form @submit.prevent="addData()">
                    <q-card-section class="q-pt-none">
                        ...
                        <span class="h_lable ">Luas WIlayah Wilayah</span>
                        <q-input v-model="form.luas" outlined square :dense="true" class="bg-white margin_btn" /> 
                        <div class="inputbs"></div>
                        <span class="h_lable ">Batas Utara</span>
                        <q-input v-model="form.utara" outlined square :dense="true" class="bg-white margin_btn" /> 
                        <span class="h_lable ">Batas Barat</span>
                        <q-input v-model="form.barat" outlined square :dense="true" class="bg-white margin_btn" />
                        <span class="h_lable ">Batas Selatan</span>
                        <q-input v-model="form.selatan" outlined square :dense="true" class="bg-white margin_btn" />
                        <span class="h_lable ">Batas Timur</span>
                        <q-input v-model="form.timur" outlined square :dense="true" class="bg-white margin_btn" />
...
export default {
  ...
  methods: {
    ...
    getView : function(){
      ...
      fetch(this.$store.state.url.URL_ADM_KECAMATAN_BATAS_WILAYAH + "view", {
          method: "POST",
          headers: {
    ...
    addData : function(number) {
      ...
      fetch(this.$store.state.url.URL_ADM_KECAMATAN_BATAS_WILAYAH + "addData", {
          method: "POST",
          headers: {
            "content-type": "application/json",
            authorization: "kikensbatara " + localStorage.token
    ...
    editData : function(){
      fetch(this.$store.state.url.URL_ADM_KECAMATAN_BATAS_WILAYAH + "editData", {
          method: "POST",
          headers: {
            "content-type": "application/json",
            authorization: "kikensbatara " + localStorage.token
    ...
    removeData : function(E){
      fetch(this.$store.state.url.URL_ADM_KECAMATAN_BATAS_WILAYAH + "removeData", {
          method: "POST",
          headers: {
            "content-type": "application/json",
            authorization: "kikensbatara " + localStorage.token
