      ...
      <q-card-section class="bg-grey-7 text-white">
        <div class="row">
          <div class="col-12 col-md-6">
            <div class="text-h6 h_titleHead">Misiz</div>
            <div class="text-subtitle2">Periode (2021-2026)</div>
          </div>
        ...
        <div class="tbl_responsive">
          ...
          <table width="100%">
            <tr class="h_table_head bg-grey-5">
              ...
              <th width="45%" class="text-center">Visi</th>
              <th width="40%" class="text-center">Misi</th>
              <th width="10%"></th>
            </tr>
            <tr class="h_table_body" v-for="(data, index) in list_data" :key="data.id">
              <td class="text-center h_top">{{ indexing(index + 1) }}.</td>
              <td class="h_top">{{ data.judul }}</td>
              <td class="h_top">{{ data.visi_uraian }}</td>
      ...
      <q-card class="mdl-md">
        ...
        <form @submit.prevent="addData()">
          <q-card-section class="q-pt-none">
            <br />
            <span class="h_lable">Pilih Visi</span>
            <select v-model="form.visi" class="inputbs">
              <option
                v-for="data in $store.state.listVisi"
                :key="data.id"
                :value="data.id"
              >
                {{ data.judul }}
            ...
            <span class="h_lable">Uraian Misi</span>
            <q-input
              v-model="form.judul"
              outlined
              square
    ...
    <q-dialog v-model="mdl_edit" persistent>
      <q-card class="mdl-md">
        ...
        <q-card-section class="q-pt-none">
          ...
          <span class="h_lable">Pilih Visi</span>
          <select v-model="form.visi" class="inputbs">
            <option v-for="data in $store.state.listVisi" :key="data.id" :value="data.id">
              {{ data.judul }}
            </option>
          </select>
          <div class="inputbs"></div>
    ...
    <q-dialog v-model="mdl_lihat" persistent>
      <q-card class="mdl-md">
        <q-card-section class="bg-blue-4">
          <div class="text-h6 h_modalhead">Detil Misi</div>
        </q-card-section>
...
export default {
  ...
  methods: {
    getView: function () {
      ...
      fetch(this.$store.state.url.URL_ADM_MISI + "view", {
        method: "POST",
        headers: {
    ...
    addData: function (number) {
      ...
      fetch(this.$store.state.url.URL_ADM_MISI + "addData", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          authorization: "kikensbatara " + localStorage.token,
    ...
    editData: function () {
      this.form.keterangan = document.getElementById("editor").value;
      fetch(this.$store.state.url.URL_ADM_MISI + "editData", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          authorization: "kikensbatara " + localStorage.token,
    ...
    removeData: function (E) {
      fetch(this.$store.state.url.URL_ADM_MISI + "removeData", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          authorization: "kikensbatara " + localStorage.token,
  ...
  mounted() {
    ...
    this.getView();
    FETCHING.getVisi(this.form.des_kel);
